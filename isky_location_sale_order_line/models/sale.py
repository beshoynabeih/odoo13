...
class SaleOrderLine(models.Model):
    ...
    def default_get(self, fields):
        ...
        if 'location_id' in fields and not res.get('location_id'):
            operation_type = self.env['stock.picking.type'].search([('code', '=', 'incoming'),
                                                                    ('default_operation_type', '=', True)], limit=1)
            res['location_id'] = operation_type.warehouse_id.lot_stock_id.id
